...
I recently took on this problem and built a reliable sequence generator in Go. In this post, I’ll walk you through my approach—step by step—including how I used:

- **Go routines and channels** for safe, concurrent sequence generation
- **SQLite** to persist the last-used value for crash recovery
- A lightweight **HTTP server** to expose the sequence
...
> 💡 **Note**: I initialized `lastSeq` to `1`, so the first call returns `3`. If you want the sequence to start at `1`, just set `lastSeq = -1`.

---

## Step 3: Adding Persistent Storage with SQLite

To survive crashes or restarts, we need to persist the sequence state. I chose **SQLite** because it’s:

- **Self-contained**: The database lives in a single file alongside your app—no external dependencies.
...
> 🛑 **Note**: Make sure you’ve imported the SQLite driver (e.g., `_ "github.com/mattn/go-sqlite3"`).

### creating the odd_sequence table if not exists
...
if err != nil {
    return nil, fmt.Errorf("failed to ensure odd_sequence table: %w", err)
}
```

we added to `IF NOT EXISTS` clause to ensure we don't make a `odd_sequence` table if the table is exist
...
```go
func SequenceOdd(db *sql.DB) (func() int, error) {
    // Ensure the sequence table exists (we moved this here so setupDB only opens/pings)
    _, err := db.Exec(`
        CREATE TABLE IF NOT EXISTS odd_sequence (
            id INTEGER PRIMARY KEY,
            created_at INTEGER
...
### Testing Update main with a saved data in DB
...
func main() {
    ...
    // initializing and getting the sequence from db
    seq, err := SequenceOdd(db)
    if err != nil {
        log.Fatal(err)
    }
...
we run the program for the second time
```sh
$ go run main.go
9
11
...
It correctly resumes from where it left off—because the last used ID is stored in `seq.db`.

---

## Step 4: Adding HTTP Server to accessing sequence

let's add simple http server in the main
...
func main() {
    ...
    // setup a get handler for /sequence endpoint
    http.HandleFunc("/sequence", func(w http.ResponseWriter, r *http.Request) {
        fmt.Fprintf(w, "%d", seq())
    })

    // running at server port 8080
    log.Fatal(http.ListenAndServe(":8080", r))
}
```

and you can verify it via curl

```sh
curl http://localhost:8080/sequence
```

and now you get the reliable sequence generator 

## Summary

In this post we built a crash-resilient, concurrent-safe odd-number sequence generator in Go.
